...
class MotionNorm(Dataset):
    def __init__(self, config, subset_name, data_path=None, extra_data_dir=None):
        ...
        self.label_i = labels
        self.len = len(self.label_i)  # 1452
        ...
        for i, motion in enumerate(motions):  # NOTE: motion: (32, 132)
            label = labels[i]
            anim = AnimationData(motion, skel=self.skel)

            if label not in self.labels:  # when new label is added,
                self.labels.append(label)  # add label to the list
                self.data_dict[label] = []  # initialize label as key
            self.data_dict[label].append(i)  # data_dict: label <-> data index set with that label
            self.motion_i.append(anim)
            self.foot_i.append(anim.get_foot_contact(transpose=True))  # [4, T]
            content.append(anim.get_content_input())  # -> (128, 32)
            style3d.append(anim.get_style3d_input())  # -> (64, 32)
            view_angles, scales = [], []
            for v in range(10):
        ...
        for key, raw in zip(["content", "style3d", "style2d"], [content, style3d, style2d]):
            ...
            pre_computed = prefix is not None  # False when train mode
            if prefix is None:   # here when train mode
                prefix = subset_name  # prefix = "train" when train mode
            norm_data.append(NormData(prefix + "_" + key, pre_computed, raw,
                                      config, extra_data_dir, keep_raw=(key != "style2d")))
            # NOTE: ("train_${content/style3d/style2d}", False, content/style3d/style2d, ...)
    ...
    def __getitem__(self, index):
        ...
        data = {"label": label,
                ...
                "contentraw": self.content.get_raw(index),  # for visualization # NOTE: (128, 32)
                "style3draw": self.style3d.get_raw(index),  # positions are used as the recon target # NOTE: (64, 32)
                "same_style3d": self.style3d.get_norm(index_same),
                "diff_style3d": self.style3d.get_norm(index_diff),
                }  # NOTE: one data point format

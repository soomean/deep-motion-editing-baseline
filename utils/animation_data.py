...
class AnimationData:
    ...
    Canonical Representation:
        Skeleton
        [32, 132] = [T, Jo * 4 + 4 global params (pos+rot) + 4 foot_contact]
    """
    def __init__(self, full, skel=None, frametime=1/30):
        ...
        self.len = len(full)  # NOTE: 32
        self.rotations = full[:, :-8].reshape(self.len, -1, 4)  # [32, 124] -> [32, 31, 4] = [T, Jo, 4]
        assert self.rotations.shape[1] == len(self.skel.topology), "Rotations do not match the skeleton."
        self.rotations /= np.sqrt(np.sum(self.rotations ** 2, axis=-1))[..., np.newaxis]  # quaternion normalization
    ...
    def get_content_input(self):
        ...
        return np.concatenate((rotations, self.rt_pos, self.rt_rot), axis=-1).transpose(1, 0)  # [Jo * 4 + 3 + 1, T] = [128, 32]
    ...
    def get_style3d_input(self):
        ...
        return np.concatenate((pos3d, self.rt_pos, self.rt_rot), axis=-1).transpose(1, 0)  # [(J - 1) * 3 + 3 + 1, T] = [64, 32]
